...
const Products = ({ products, setProducts, user, setUser }) => {
  ...
  const handleUpdate = async () => {
    try {
      const updatedProduct = await fetch(
        `https://6556162184b36e3a431efcfa.mockapi.io/api/products${editProduct.id}`,
        {
          method: "PUT",
          headers: {
  ...
  const handleDelete = async (productId) => {
    try {
      await fetch(
        `https://6556162184b36e3a431efcfa.mockapi.io/api/products${productId}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
